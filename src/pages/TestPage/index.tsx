    ...
    <div className="w-full">
      ...
      <div className="pt-10 lg:px-32 md:px-32 max-md:px-20 sm:px-5 max-sm:px-3 pb-14">
        {testList?.map((item, index) => (
          ...
          >
            <div className="flex max-sm:flex-col items-center sm:gap-5 max-sm:gap-3">
              <img src={item.imageSrc} alt="" />
              <p className="lg:text-2xl md:text-2xl max-md:text-xl max-sm:text-xs text-primary">
                {item.testName}
              </p>
            </div>
            {item.status === 'new' && (
            ...
            {item.status === 'done' && (
              <div className="flex items-center gap-4  max-lg:flex-col">
                <Tag className="bg-secondPrimary text-primary font-bold py-3 px-6 rounded-xl">
                  {item.score}/100
                </Tag>
                <Button
                  type="outline"
